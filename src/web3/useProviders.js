...
function getFallbackProvider() {
  ...
  if (process.env.REACT_APP_INFURA_API_KEY)
    providerKeys.infura = process.env.REACT_APP_INFURA_API_KEY;
  if (process.env.REACT_APP_ALCHEMY_API_KEY)
    providerKeys.alchemy = process.env.REACT_APP_ALCHEMY_API_KEY;
  if (process.env.REACT_APP_ETHERSCAN_API_KEY)
    providerKeys.etherscan = process.env.REACT_APP_ETHERSCAN_API_KEY;
...
function useProvider() {
  ...
  const reloadedProvider = useListeners(
  ...
  useEffect(() => {
    ...
    if (
      ...
      (web3Provider.signerOrProvider &&
        supportedChains().includes(
          parseInt(web3Provider.signerOrProvider.chainId)
        ))
    ) {
...
export { useProvider, connect, logout };
